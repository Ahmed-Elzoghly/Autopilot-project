...
benchmark_all = load('Benchmark_B747_FC5.mat');
...
% Run simulations for each delta_control input
for i = 1:9
    % Run RK4 Simulation
    for j = 2:n+1
        [Forces(:,j), Moments(:,j)] = Airframe_Model(states_0, states_RK4(:,j-1), wdot(j-1), mass, Inertia, gravity, delta_control(i,:), Matrix_states, Matrix_controls);
    ...
    % Run Simulink
    Simulator = sim('Simulator21.slx');

    % Plot and save figures
    plot_Validation(t_vec, states_RK4, Simulator, delta_control(i,:), benchmark_all);
end
